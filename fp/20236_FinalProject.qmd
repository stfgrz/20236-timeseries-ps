...
setwd("C:/Users/USER/Desktop/TS giocarro")
...
plot(ts_data,
     main = "Original Monthly Temperature Series (GISTEMP)",
     ylab = "Temperature (°C)",
     xlab = "Time",
     col = "darkblue",
     lwd = 1)
...
#We deseasonalize the time series
deseasoned_stl <- ts_data - seasonal_stl
plot(deseasoned_stl)


# 1. Specify the HMM with 2 states
hmm_2 <- depmix(deseasoned_stl ~ 1, 
                family = gaussian(), 
                nstates = 2, 
                data = data.frame(deseasoned_stl))
...
plot(time(deseasoned_stl), estStates$state, 
     type = "l", 
...
estMeans[estStates$state == ii] <- estMean2
...
results_df <- data.frame(
  time_index = as.numeric(time(deseasoned_stl)),
  sample_trajectory = as.numeric(deseasoned_stl),
...
plotobj <- ggplot(results_df, aes(x = time_index, y = value)) +
  ...
  theme_minimal() +
  labs(title = "HMM States and Deseasonalized Series", x = "Time", y = "Value")
...
# Get time and values
time_vals <- as.numeric(time(deseasoned_stl))
values <- as.numeric(deseasoned_stl)
...
df <- data.frame(
  temp = as.numeric(deseasoned_stl),
  time = time_vals
)

# Specify HMM with linear trend in each state: response ~ time
hmm_linear <- depmix(temp ~ time, 
                     family = gaussian(), 
                     nstates = 2, 
                     data = df)
