...
# Create the plot
plotobj <- ggplot(results_df, aes(x = time_index, y = value)) +
  geom_line() +
...
initRW <- log(c(var(ts_gistemp), var(ts_gistemp) / 100))

# Fit model
fitRW  <- dlmMLE(ts_gistemp, parm = initRW, build = buildRW)
modRW  <- buildRW(fitRW$par)
```

```{r}
filtRW <- dlmFilter(ts_gistemp, modRW)
...
# Plot
ggplot(df_rw, aes(x = Time)) +
  geom_line(aes(y = Anomaly), color = "orange") +
  geom_line(aes(y = Level_smth), color = "steelblue") +
  geom_ribbon(aes(ymin = Lower95, ymax = Upper95),
              fill = "steelblue", alpha = 0.2) +
  labs(title = "RW + Noise: Smoothed Level + 95% CI",
...
stl_decomp_tmin <- stl(y[,1], s.window = "periodic", robust = TRUE)
stl_decomp_tmax <- stl(y[,2], s.window = "periodic", robust = TRUE)

seasonal_comp_tmin <- stl_decomp_tmin$time.series[, "seasonal"]
seasonal_comp_tmax <- stl_decomp_tmax$time.series[, "seasonal"]
y_seas      <- cbind(seasonal_comp_tmin, seasonal_comp_tmax)
y_deseas <- y - y_seas
y <- y_deseas
...
res_rawCF <- rawCF$res     # n×2 ts of one-step-ahead errors y_t - ŷ_{t|t-1}
pred_seCF <- rawCF$sd      # n×2 ts of predictive standard deviations

# reconstruct the in‐sample forecasts 
y_predCF <- y - res_rawCF


okCF  <- complete.cases(res_rawCF)   # drop initial NA
eCF   <- res_rawCF[okCF,]              # raw errors
seCF  <- pred_seCF[okCF,]              # predictive SD

rmse <- sqrt(colMeans(eCF^2))
mae  <- colMeans(abs(eCF))

lower <- y_predCF - 1.96 * pred_seCF
upper <- y_predCF + 1.96 * pred_seCF
cov  <- sapply(1:2, function(i)
  mean(temp[ok,i] >= lower[okCF,i] & y[okCF,i] <= upper[okCF,i]) * 100
)
```
...
ggplot() +
  ...
  scale_color_manual(
    name = "",
    values = c("Observed" = "black", "Forecast mean" = "firebrick")
  ) +
  coord_cartesian(
