...
# 2. Smoothed means (drop init)
m_smth_all <- smthLL2S$s[-1, ]      # matrix T×13
level_smth <- m_smth_all[, 1]       # length T

# 3. Extract level variances at each time
...
# 4. Build 95% bands
upper95_smth <- level_smth + 1.96 * sd_level
lower95_smth <- level_smth - 1.96 * sd_level
...
df_smth <- data.frame(
  ...
  Upper95    = upper95_smth,
  Lower95    = lower95_smth
)
...
# 1. MLE with Hessian (ATTENTO CHE CI METTE UN BOTTO, VALUTIAMO SE FARE SU R ONLINE CHE QUI IL TOSTAPANE SI FONDE - STEFANO & TOSTAPANE)
...
```{r}
#| label: Forecast‐performance on a hold‐out
...
Namely, consider the bivariate time series of minimum and maximum temperature, \[ Y_t =
