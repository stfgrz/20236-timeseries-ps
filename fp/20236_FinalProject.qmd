...
graph_1 <- ggplot(ts_df, aes(x = date, y = value)) +
  # Map the "raw data" line to a color‐label "Data"
  geom_line(aes(color = "Data"), linewidth = 0.25) +
  # Map the LOESS fit to a color‐label "Smoothed Trend"
  geom_smooth(
  ...
  # We can choose whether to include the horizontal line in the legend.
  # If you do want it in the legend, map it similarly to a label inside aes(color=...). 
  # Otherwise, keep it outside aes() so it does not appear in the legend.
  geom_hline(
    yintercept = 0,
    color = "gray20",
    linetype = "dashed",
  ...
  labs(
    ...
    color = NULL   # Remove the default legend title
  ) +
  scale_color_manual(
    # Match the aes(color=…) labels to actual colors:
    values = c(
      "Data" = "orange",
...
print(HMM_2s)
ggsave("HMM_2s.pdf", plot = HMM_2s, device = "pdf", width = 10, height = 6, units = "in")
```
...
### 2.2.a. Independent Random Walk plus Noise
...
ggsave("ggplot_df_rwpn_sub.pdf", plot = ggplot_df_rwpn_sub, device = "pdf", width = 12, height = 6, units = "in")
```
...
print(overlay_plot_ci)
ggsave("overlay_plot_cipdf.pdf", plot = overlay_plot_ci, device = "pdf", width = 12, height = 6, units = "in")
```
...
print(p_qq)
ggsave("p_qq_rwpn.pdf", plot = p_qq, device = "pdf", width = 8, height = 6, units = "in")

# Print and save ACF plot
print(p_acf)
ggsave("p_acf_rwpn.pdf", plot = p_acf, device = "pdf", width = 8, height = 6, units = "in")
```
...
print(graph3am)
ggsave("graph3am.pdf", plot = graph3am, device = "pdf", width = 12, height = 6, units = "in")
```
...
install.packages(c("dlm", "tidyverse", "lubridate", "geosphere"))
...
ggsave("spatial_forecast.pdf", plot = spatial_forecast, device = "pdf", width = 12, height = 6, units = "in")
```
