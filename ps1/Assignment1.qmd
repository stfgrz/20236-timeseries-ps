...
#### NOTES
  ...
  Parlare di temperatura globale.
...
graph_4 <- ggplot(seasonal_indices, aes(x = Month, y = Seasonal, group = Period, color = Period)) +
  ...
  ggtitle("Evolution of Seasonal Component (30-Year Windows)") +
...
#### NOTES
  ...
  Non sappiamo come trattare l'overlap, probabilmente non ci sono conseguenze rilevanti
  
  Ha molto senso se si considera fino al 1950 (anno di arrivo, considera 1949), in quanto quando aumenta il trend nel 1950 -ca si mischiano gli effetti.
  
  Valuta modello con cambio a 1960-1970
  
  Come seasonality qualcuno si aspetta estati più caldi dell'inverno 
  
  


## 1.4 Noise and Uncertainty:
...
It appears evident that the mother of whoever was resposible for the observation and reporting of the outliers was a puttana; we will hence need to do minimal data clearning:

```{r}
filtered_residuals_dm_df <- residuals_dm_df %>%
  filter(value > quantile(value, 0.12, na.rm = TRUE) & value < quantile(value, 0.92, na.rm = TRUE))
...
```{r}
acf(na.omit(residuals_da), main="ACF of residuals - additive decomposition")
acf(na.omit(residuals_dm), main="ACF of residuals - multiplicative decomposition")
acf(na.omit(residuals_stl), main="ACF of residuals - stl decomposition")
```
...
#I remove the first observation
ts_data_sub_removed <- ts_data_sub[-1]
...
#alpha 0.1
HWdata_01 <- HoltWinters(ts_data_sub, alpha=.1, beta=F, gamma=F)
plot(HWdata_01)
#alpha 0.3
HWdata_03 <- HoltWinters(ts_data_sub, alpha=.3, beta=F, gamma=F)
plot(HWdata_03)
#alpha 0.1
plot(HWdata)
#alpha 0.7
HWdata_07 <- HoltWinters(ts_data_sub, alpha=.7, beta=F, gamma=F)
plot(HWdata_07)
#alpha 0.9
HWdata_09 <- HoltWinters(ts_data_sub, alpha=.9, beta=F, gamma=F)
plot(HWdata_09)
...
where the seasonal component is a dummy for all the seasonal variables
...
-   The 1st plot compares the flat prediction with the observed data
-   The 2nd plot show the raw forecast error.
-   The 3rd plot shows the error in absolute value
-   The 4th plot shows the absolute percentage error
-   The 5th plot shows the squared error

Interpretation: the forecast error increases as time goes on as the trend fitting the general data seems to show an average increase in the deviation from the baseline value. Hence, it is plausible that assuming the flat prediction stemming from 1880-1930 underestimates temperatures of following periods.
